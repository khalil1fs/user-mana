...
ngOnInit(): void {
    ...
    this.etatEtapeCampagneService.findAll().subscribe((data) => this.etatEtapeCampagnes = data);
        ...
        this.encadrementEtudiantEnjeuxIrdsListe.forEach((element, index) => {
            if (element === p) { this.encadrementEtudiantEnjeuxIrdsListe.splice(index, 1); }
        });
    }
        ...
        this.encadrementEtudiantDisciplineScientifiquesListe.forEach((element, index) => {
            if (element === p) { this.encadrementEtudiantDisciplineScientifiquesListe.splice(index, 1); }
        });
    }
