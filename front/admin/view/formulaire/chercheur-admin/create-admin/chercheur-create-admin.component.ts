...
// methods
    ...
    addDomaineScientifiqueChercheurs() {
    ...
    addInstrumentsEtDispositifsIrdChercheurs() {
    ...
    public async saveWithShowOption(showList: boolean) {
        if(this.selectedChercheur.password === this.selectedChercheur.confirmPassword) {
            if (this.emailValidationRegex.test(this.selectedChercheur.email)) {
                await this.chercheurService.save().subscribe(chercheur => {
                    console.log('email: ' + chercheur.email);
                        this.chercheurs.push({...chercheur});
                        this.users.push({...chercheur});
